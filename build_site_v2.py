...
<div class="section-heading"><p class="eyebrow">{eyebrow}</p><h2>{heading}</h2>{f'<p class="section-intro">{intro}</p>' if intro else ''}</div>{body}</div></section>'''
...
def steps(items):
    return '<div class="steps">' + ''.join(f'<div class="step"><span class="step-number">{i:02}</span><h3>{h}</h3><p>{p}</p></div>' for i, (h, p) in enumerate(items, 1)) + '</div>'
...
def cta(heading='What keeps landing back on your desk?', text='Tell me about one recurring task, the tools involved, and where it gets stuck. We can work out whether there is a useful first step.', need='Agentic OS', label='Talk through your workflow'):
    return section('Start with a conversation', heading, action(label, need), text, 'section-sage')


def hero(eyebrow, heading, intro, buttons=''):
    return f'<section class="page-hero"><div class="container page-hero-inner"><p class="eyebrow">{eyebrow}</p><h1>{heading}</h1><p>{intro}</p><div class="hero-actions">{buttons}</div></div></section>'


def faq(items):
    return section('Questions, answered', 'Before you get started', '<div class="faq-list">' + ''.join(f'<details><summary>{q}</summary><p>{a}</p></details>' for q,a in items) + '</div>')
...
WORKFLOW = '''<figure class="workflow-preview"><figcaption><span class="status-dot" aria-hidden="true"></span> How an enquiry could move through Agentic OS</figcaption><ol><li><span class="workflow-label">INCOMING</span><strong>A new enquiry arrives</strong><p>Name, request, and contact details in your inbox.</p></li><li><span class="workflow-label">PREPARED FOR YOU</span><strong>The context and draft are together</strong><p>The agent gathers relevant records, drafts a reply, and proposes the next task.</p></li><li class="approval-step"><span class="workflow-label">YOUR DECISION</span><strong>Review. Edit. Approve.</strong><p>You decide what reaches the customer.</p></li></ol><p class="workflow-caption">Illustrative workflow. The steps and permissions are agreed for your business.</p></figure>'''

HOME = '''<section class="hero"><div class="container hero-grid"><div><p class="eyebrow">Agentic OS for owner-led businesses</p><h1 class="display">AI workflows for the work<br><span class="accent">you keep doing by hand.</span></h1><p class="hero-intro">I build AI workflows that gather the context, prepare the reply, and keep track of the next step. You spend less time moving work between tools, and stay in charge of what happens next.</p><div class="hero-actions">''' + action() + link('Explore Agentic OS', '/services/automation/', 'btn btn-outline light') + '''</div><p class="hero-note">Built by Matthew Baggetta in Welland. Start with one workflow.</p></div>''' + WORKFLOW + '</div></section>'
HOME += section('The work behind the work', 'Too much of your day goes into getting ready to do the job.', steps([
...
]), 'Agentic OS connects AI agents with the tools and information your business already uses. Each workflow has a defined job, access limits, and a place for your approval.', 'section-paper')
HOME += section('Start where it helps', 'A useful first workflow beats a grand AI overhaul.', '<div class="prose"><p>We look at one recurring task, what it costs you in time, and the judgment it needs. Then I build and test a bounded workflow around it. You see what it does, where it pauses, and what happens when something goes wrong.</p><p>Some steps need AI. Others need a simple rule or a better process. The choice follows the job.</p>' + link('See how an Agentic OS engagement works', '/services/automation/#process') + '</div>', theme='section-sage')
...
OPS = hero('Agentic OS', 'Routine work, ready for your judgment.', 'Agentic OS is a custom service that connects AI agents, business knowledge, and your existing tools into repeatable workflows. It can prepare an inbox brief, draft a client reply, or turn a meeting into proposed tasks. You set the boundaries and review the actions that need your judgment.', action())
OPS += section('What it can cover', 'Choose a job that keeps coming back.', cards([
...
OPS += section('A workflow in practice', 'The agent prepares. You decide.', WORKFLOW, 'For customer communication, the system brings the draft and supporting context to you. Nothing in this example sends itself.', 'section-dark')
OPS += section('How we build it', 'Start with one workflow.', steps([
...
SERVICES = hero('Services', 'Help with the work your business depends on.', 'Agentic OS, websites, SEO, and AI visibility. Each service has its own starting point and scope. You do not need to buy all four.', action())
SERVICES += section('Choose your starting point', 'What needs to work better?', cards([
...
def pricing():
    ...
    return section('Published website plans', 'A website, with ongoing support for growth.', body, 'Choose the level of website, search, and content support you need. We confirm the scope and terms before you commit.', ident='plans')
...
WEBSITES = hero('Websites for local businesses', 'Give your next customer a clear reason to call.', 'Your services, your work, and the details customers need to choose you. I build custom websites for businesses in Niagara and the GTA, with clear copy, mobile layouts, and a direct path to a call, booking, or quote.', link('Get a free website preview', '#preview', 'btn btn-primary') + link('See website plans', '#plans', 'btn btn-outline light'))
WEBSITES += section('Built around your business', 'The useful details come first.', cards([
...
WEBSITES += pricing()
WEBSITES += section('Website only', 'Need a build without a monthly plan?', '<div class="prose"><p>Published one-time options: Express $297, Starter $497, Standard $997, and Premium $1,497. We confirm the pages, content, and functionality that fit the build before you buy.</p><p>A larger site or custom integration is scoped separately.</p></div>')
...
for path, title, heading, intro, items in [
    ...
    body += section('Choose a practical starting point', 'Build around the customer’s decision.', steps(items), theme='section-paper')
    body += section('Prices and inclusions', 'Compare the current website plans.', '<div class="prose"><p>One-time builds and monthly website plans are listed together, with their setup costs and inclusions.</p>' + link('See website plans and pricing', '/services/websites/#plans') + '</div>')
...
SEO = hero('SEO and local search', 'Be easier to find when customers need what you do.', 'SEO improves the content, technical structure, and business information that help search engines understand your website. I work on the pages and queries that matter to your services, then report what changes.', action('Talk about search', 'SEO and search'))
SEO += section('The work', 'Build a stronger search foundation.', steps([('Technical foundations', 'Check crawlability, indexing, site structure, mobile performance, and the issues that keep useful pages from being found.'), ('Useful pages', 'Write and improve service content around real customer questions. Use clear titles, headings, and internal links.'), ('Local presence', 'Connect your website with accurate Google Business Profile details, service information, and consistent local listings.'), ('Measurement', 'Track search visibility and customer actions where tracking is available. Explain what changed and what still needs work.')]), theme='section-paper')
SEO += section('Relevant experience', 'Search and content work through Jetta Grove.', '<div class="prose"><p>My prior consultancy work covers SEO, content, and growth marketing for technology businesses. I bring that experience to the service pages, customer questions, and local searches that matter to your business.</p>' + link('About Matthew', '/about/') + '</div>')
...
ENGAGEMENTS = hero('How it works', 'Agree the job before building the system.', 'A useful first conversation identifies what is getting in the way, what you already use, and what an appropriate first step could be. You work directly with me throughout.', action())
ENGAGEMENTS += section('Choose the right engagement', 'The process follows the work.', steps([('Agentic OS', 'Map one recurring workflow, define the access and approval points, and agree the implementation and testing scope. Ongoing monitoring and support are agreed separately.'), ('Website', 'Request a free preview, then choose the build and confirm the content. Monthly website plans and one-time options have published prices.'), ('Search and AI visibility', 'Review the current site and business goals. Agree the technical, content, and measurement work based on the market and available evidence.')]), theme='section-paper')
...
WORK += section('Concept builds', 'Design examples for local businesses.', '<div class="bento"><article class="bento-card half"><img src="/assets/jk-motors-preview.png" alt="JK Motors auto-repair website concept" width="1200" height="800" loading="lazy"><p class="eyebrow">Concept build</p><h3>JK Motors</h3><p>Repair services, business details, and a clear call-first layout. A design concept, not a client engagement.</p>' + link('View the concept', 'https://immeasurablematt.github.io/jk-motors/') + '</article><article class="bento-card half"><img src="/assets/evelyns-sandwich-factory-preview.png" alt="Evelyn’s Sandwich Factory website concept" width="1200" height="800" loading="lazy"><p class="eyebrow">Concept build</p><h3>Evelyn’s Sandwich Factory</h3><p>A phone-friendly menu, hours, location, and ordering path. A design concept, not a client engagement.</p>' + link('View the concept', 'https://immeasurablematt.github.io/evelyns-sandwich-factory/') + '</article></div>')
WORK += section('Prior consultancy work', 'Jetta Grove Consulting', '<div class="prose"><p>My earlier work through Jetta Grove covered SEO, content, and growth marketing for technology businesses. This is prior consultancy experience, separate from Well and Good client work.</p>' + link('Visit Jetta Grove', 'https://www.jettagrove.com/') + '</div>', theme='section-sage')
WORK += section('Agentic OS', 'See how an enquiry could become a prepared reply.', '<div class="prose"><p>Explore an illustrative workflow from the first enquiry to a draft reply, with the supporting context and a place for you to review the next step.</p>' + link('Explore the workflow', '/services/automation/') + '</div>') + cta()
...
page('/frank-baggetta/', 'Frank Baggetta Website | Well and Good', 'A live website for Toronto musician Frank Baggetta, bringing his performances, services, and contact path together.', hero('Live website client', 'A website for the performance, and the enquiry.', 'Frank Baggetta’s site brings his music, photographs, and service details together so people can understand his work and get in touch.', link('Visit frankbaggetta.ca', 'https://frankbaggetta.ca/', 'btn btn-primary')) + section('The website', 'Help visitors decide, then enquire.', '<div class="proof-row"><div class="prose"><p>The site presents performance videos, information about events, and a direct contact path.</p><p>Visitors can hear Frank perform, explore his services, and get in touch about their event.</p>' + link('See more work', '/work/') + '</div><img src="/assets/frankbaggetta-preview.webp" alt="Frank Baggetta website" width="1200" height="800" loading="lazy"></div>', theme='section-paper') + cta('Have a website in mind?', 'See a private preview before deciding whether to buy.', 'Website preview', 'Request a website preview'))

CONTACT = hero('Contact', 'Tell me what keeps coming back to you.', 'A recurring task, a website that needs work, or a search problem. Share a few details and I’ll reply personally to discuss a useful next step.')
CONTACT += section('Start here', 'A conversation about the work.', '<div class="contact-layout"><div class="prose"><p>For Agentic OS, describe one task, the tools it touches, and where you get stuck. For a website, send a link and what customers should be able to do.</p><p>The initial conversation is free. This form sends an enquiry; I’ll reply to arrange a time or ask for the details needed to help.</p><p>Prefer email? <a href="mailto:' + EMAIL + '">' + EMAIL + '</a></p></div><div class="contact-panel">' + form() + '</div></div>', theme='section-paper', ident='book')
